...
extern lzma_ret
lzma_delta_coder_init(lzma_next_coder* next, const lzma_allocator* allocator, const lzma_filter_info* filters) {
...
lzma_delta_coder_memusage(const void* options) {
  ...
  if(opt == NULL || opt->type != LZMA_DELTA_TYPE_BYTE || opt->dist < LZMA_DELTA_DIST_MIN ||
