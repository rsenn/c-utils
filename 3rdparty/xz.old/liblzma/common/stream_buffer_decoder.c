...
extern LZMA_API(lzma_ret) lzma_stream_buffer_decode(uint64_t* memlimit,
  ...
  // Sanity checks
  if(in_pos == NULL || (in == NULL && *in_pos != in_size) || *in_pos > in_size || out_pos == NULL ||
    ...
    return LZMA_PROG_ERROR;
  {
    ...
    lzma_next_coder stream_decoder = next_init;
    lzma_ret ret = lzma_stream_decoder_init(&stream_decoder, allocator, *memlimit, flags);
    ...
    if(ret == LZMA_OK) {
      ...
      ret = stream_decoder.code(
          stream_decoder.coder, allocator, in, in_pos, in_size, out, out_pos, out_size, LZMA_FINISH);
