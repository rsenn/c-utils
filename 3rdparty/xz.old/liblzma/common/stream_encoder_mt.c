...
worker_encode(worker_thread* thr, worker_state state) {
  ...
  ret = lzma_block_encoder_init(&thr->block_encoder, thr->allocator, &thr->block_options);
  if(ret != LZMA_OK) {
    worker_error(thr, ret);
    return THR_STOP;
  }
  ...
  do {
    mythread_sync(thr->mutex) {
      ...
      while(in_size == thr->in_size && thr->state == THR_RUN) mythread_cond_wait(&thr->cond, &thr->mutex);
  ...
  switch(ret) {
    ...
    case LZMA_OK: {
      ...
      mythread_sync(thr->mutex) {
        while(thr->state == THR_RUN) mythread_cond_wait(&thr->cond, &thr->mutex);
      ...
      ret = lzma_block_uncomp_encode(
          &thr->block_options, thr->in, in_size, thr->outbuf->buf, &thr->outbuf->size, out_size);
...
threads_stop(lzma_stream_coder* coder, bool wait_for_threads) {
  ...
  for(uint32_t i = 0; i < coder->threads_initialized; ++i) {
    mythread_sync(coder->threads[i].mutex) {
      while(coder->threads[i].state != THR_IDLE) mythread_cond_wait(&coder->threads[i].cond, &coder->threads[i].mutex);
    }
  }
...
static lzma_ret
initialize_new_thread(lzma_stream_coder* coder, const lzma_allocator* allocator) {
...
static lzma_ret
stream_encode_in(lzma_stream_coder* coder,
                 const lzma_allocator* allocator,
  ...
  while(*in_pos < in_size || (coder->thr != NULL && action != LZMA_RUN)) {
    ...
    lzma_bufcpy(in, in_pos, in_size, coder->thr->in, &thr_in_size, coder->block_size);
    ...
    const bool finish = thr_in_size == coder->block_size || (*in_pos == in_size && action != LZMA_RUN);
    ...
    if(block_error) {
      ...
      mythread_sync(coder->mutex) { ret = coder->thread_error; }
...
static bool
wait_for_work(lzma_stream_coder* coder, mythread_condtime* wait_abs, bool* has_blocked, bool has_input) {
  ...
  mythread_sync(coder->mutex) {
    ...
    //  - Time out occurs.
    while((!has_input || coder->threads_free == NULL || !lzma_outq_has_buf(&coder->outq)) &&
          !lzma_outq_is_readable(&coder->outq) && coder->thread_error == LZMA_OK && !timed_out) {
      if(coder->timeout != 0)
        timed_out = mythread_cond_timedwait(&coder->cond, &coder->mutex, wait_abs) != 0;
      else
...
static lzma_ret
stream_encode_mt(void* coder_ptr,
                 const lzma_allocator* allocator,
                 const uint8_t* restrict in,
                 size_t* restrict in_pos,
                 size_t in_size,
  ...
  switch(coder->sequence) {
    case SEQ_STREAM_HEADER:
      lzma_bufcpy(coder->header, &coder->header_pos, sizeof(coder->header), out, out_pos, out_size);
    ...
    case SEQ_BLOCK: {
      ...
      while(true) {
        mythread_sync(coder->mutex) {
          ...
          ret = lzma_outq_read(&coder->outq, out, out_pos, out_size, &unpadded_size, &uncompressed_size);
        }
        ...
        ret = stream_encode_in(coder, allocator, in, in_pos, in_size, action);
        if(ret != LZMA_OK) {
          threads_stop(coder, false);
          return ret;
        }
        ...
        if(wait_for_work(coder, &wait_abs, &has_blocked, *in_pos < in_size))
      ...
      coder->progress_out += lzma_index_size(coder->index) + LZMA_STREAM_HEADER_SIZE;
    }
    ...
    case SEQ_INDEX: {
      ...
      const lzma_ret ret = coder->index_encoder.code(
          coder->index_encoder.coder, allocator, NULL, NULL, 0, out, out_pos, out_size, LZMA_RUN);
      if(ret != LZMA_STREAM_END)
        return ret;

      // Encode the Stream Footer into coder->buffer.
      coder->stream_flags.backward_size = lzma_index_size(coder->index);
      if(lzma_stream_footer_encode(&coder->stream_flags, coder->header) != LZMA_OK)
    ...
    case SEQ_STREAM_FOOTER:
      lzma_bufcpy(coder->header, &coder->header_pos, sizeof(coder->header), out, out_pos, out_size);
      return coder->header_pos < sizeof(coder->header) ? LZMA_OK : LZMA_STREAM_END;
  }
...
stream_encoder_mt_end(void* coder_ptr, const lzma_allocator* allocator) {
  ...
  for(size_t i = 0; coder->filters[i].id != LZMA_VLI_UNKNOWN; ++i) lzma_free(coder->filters[i].options, allocator);
...
static lzma_ret
get_options(const lzma_mt* options,
...
static lzma_ret
stream_encoder_mt_init(lzma_next_coder* next, const lzma_allocator* allocator, const lzma_mt* options) {
  ...
  if(coder->threads_max != options->threads) {
    ...
    coder->threads = lzma_alloc(options->threads * sizeof(worker_thread), allocator);
  ...
  // Output queue
  return_if_error(lzma_outq_init(&coder->outq, allocator, outbuf_size_max, options->threads));
  ...
  return_if_error(lzma_stream_header_encode(&coder->stream_flags, coder->header));
...
extern LZMA_API(lzma_ret) lzma_stream_encoder_mt(lzma_stream* strm, const lzma_mt* options) {
...
extern LZMA_API(uint64_t) lzma_stream_encoder_mt_memusage(const lzma_mt* options) {
  lzma_options_easy easy;
  const lzma_filter* filters;
  uint64_t block_size;
  uint64_t outbuf_size_max;

  if(get_options(options, &easy, &filters, &block_size, &outbuf_size_max) != LZMA_OK)
    return UINT64_MAX;
  ...
  const uint64_t outq_memusage = lzma_outq_memusage(outbuf_size_max, options->threads);
  if(outq_memusage == UINT64_MAX)
    return UINT64_MAX;

  // Sum them with overflow checking.
  uint64_t total_memusage = LZMA_MEMUSAGE_BASE + sizeof(lzma_stream_coder) + options->threads * sizeof(worker_thread);
