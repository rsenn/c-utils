...
static lzma_ret
copy_or_code(lzma_simple_coder* coder,
...
simple_code(void* coder_ptr,
  ...
  if(out_avail > buf_avail || buf_avail == 0) {
    ...
    *out_pos += buf_avail;
    ...
    {
      const lzma_ret ret = copy_or_code(coder, allocator, in, in_pos, in_size, out, out_pos, out_size, action);
  ...
  if(coder->size > 0) {
    {
      const lzma_ret ret =
          copy_or_code(coder, allocator, in, in_pos, in_size, coder->buffer, &coder->size, coder->allocated, action);
      assert(ret != LZMA_STREAM_END);
      if(ret != LZMA_OK)
        return ret;
    }
...
static lzma_ret
simple_coder_update(void* coder_ptr,
                    const lzma_allocator* allocator,
...
lzma_simple_coder_init(lzma_next_coder* next,
  ...
  if(coder == NULL) {
    ...
    coder = lzma_alloc(sizeof(lzma_simple_coder) + 2 * unfiltered_max, allocator);
    if(coder == NULL)
