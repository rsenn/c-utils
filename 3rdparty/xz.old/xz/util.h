...
extern void* xrealloc(void* ptr, size_t size) lzma_attribute((__malloc__)) lzma_attr_alloc_size(2);
...
///             if an error occurs.
///
extern uint64_t str_to_uint64(const char* name, const char* value, uint64_t min, uint64_t max);
...
extern void my_snprintf(char** pos, size_t* left, const char* fmt, ...) lzma_attribute((__format__(__printf__, 3, 4)));
