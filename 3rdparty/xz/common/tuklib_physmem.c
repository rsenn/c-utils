...
tuklib_physmem(void) {
  ...
  if((GetVersion() & 0xFF) >= 5) {
    ...
    if(kernel32 != NULL) {
      ...
      gmse_type gmse = (gmse_type)GetProcAddress(kernel32, "GlobalMemoryStatusEx");
      if(gmse != NULL) {
  ...
  if(__dpmi_get_free_memory_information(&meminfo) == 0 && meminfo.total_number_of_physical_pages != (unsigned long)-1)
