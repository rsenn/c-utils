...
static void
index_tree_node_end(index_tree_node* node,
...
static void
index_tree_end(index_tree* tree,
               const lzma_allocator* allocator,
               void (*free_func)(void* node, const lzma_allocator* allocator)) {
...
static index_stream*
index_stream_init(lzma_vli compressed_base,
                  lzma_vli uncompressed_base,
...
extern LZMA_API(void) lzma_index_end(lzma_index* i, const lzma_allocator* allocator) {
...
extern LZMA_API(uint64_t) lzma_index_memusage(lzma_vli streams, lzma_vli blocks) {
  ...
  if(streams == 0 || streams > UINT32_MAX || blocks > LZMA_VLI_MAX || streams > limit / stream_base ||
     groups > limit / group_base || limit - streams_mem < groups_mem)
    return UINT64_MAX;
...
extern LZMA_API(lzma_vli) lzma_index_block_count(const lzma_index* i) { return i->record_count; }

extern LZMA_API(lzma_vli) lzma_index_stream_count(const lzma_index* i) { return i->streams.count; }

extern LZMA_API(lzma_vli) lzma_index_size(const lzma_index* i) {
  return index_size(i->record_count, i->index_list_size);
}

extern LZMA_API(lzma_vli) lzma_index_total_size(const lzma_index* i) { return i->total_size; }

extern LZMA_API(lzma_vli) lzma_index_stream_size(const lzma_index* i) {
  // Stream Header + Blocks + Index + Stream Footer
  return LZMA_STREAM_HEADER_SIZE + i->total_size + index_size(i->record_count, i->index_list_size) +
         LZMA_STREAM_HEADER_SIZE;
}

static lzma_vli
index_file_size(lzma_vli compressed_base,
                lzma_vli unpadded_sum,
                lzma_vli record_count,
                lzma_vli index_list_size,
                lzma_vli stream_padding) {
...
extern LZMA_API(lzma_vli) lzma_index_uncompressed_size(const lzma_index* i) { return i->uncompressed_size; }
...
extern LZMA_API(lzma_ret) lzma_index_stream_flags(lzma_index* i, const lzma_stream_flags* stream_flags) {
  index_stream* s;
...
extern LZMA_API(lzma_ret) lzma_index_stream_padding(lzma_index* i, lzma_vli stream_padding) {
  index_stream* s;
...
extern LZMA_API(lzma_ret) lzma_index_append(lzma_index* i,
                                            const lzma_allocator* allocator,
  ...
  // Validate.
  if(i == NULL || unpadded_size < UNPADDED_SIZE_MIN || unpadded_size > UNPADDED_SIZE_MAX ||
  ...
  compressed_base = g == NULL ? 0 : vli_ceil4(g->records[g->last].unpadded_sum);
  uncompressed_base = g == NULL ? 0 : g->records[g->last].uncompressed_sum;
  index_list_size_add = lzma_vli_size(unpadded_size) + lzma_vli_size(uncompressed_size);
  ...
  if(g != NULL && g->last + 1 < g->allocated) {
    ...
    g = lzma_alloc(sizeof(index_group) + i->prealloc * sizeof(index_record), allocator);
    if(g == NULL)
  ...
  g->records[g->last].uncompressed_sum = uncompressed_base + uncompressed_size;
...
extern LZMA_API(lzma_ret)
    lzma_index_cat(lzma_index* restrict dest, lzma_index* restrict src, const lzma_allocator* allocator) {
  ...
  // to be done before modifying dest or src.
  {
    ...
    if(g != NULL && g->last + 1 < g->allocated) {
      ...
      newg = lzma_alloc(sizeof(index_group) + (g->last + 1) * sizeof(index_record), allocator);
      if(newg == NULL)
        return LZMA_MEM_ERROR;
...
index_dup_stream(const index_stream* src, const lzma_allocator* allocator) {
  ...
  dest = index_stream_init(
      src->node.compressed_base, src->node.uncompressed_base, src->number, src->block_number_base, allocator);
  if(dest == NULL)
    return NULL;
  ...
  destg = lzma_alloc(sizeof(index_group) + src->record_count * sizeof(index_record), allocator);
  if(destg == NULL) {
  ...
  destg->last = src->record_count - 1;
  {
    ...
    index_group* srcg = (index_group*)(src->groups.leftmost);
    size_t i = 0;
    do {
...
extern LZMA_API(lzma_index*) lzma_index_dup(const lzma_index* src, const lzma_allocator* allocator) {
  ...
  dest->index_list_size = src->index_list_size;
  {
    ...
    index_stream* srcstream = (index_stream*)(src->streams.leftmost);
    do {
...
iter_set_info(lzma_index_iter* iter) {
  ...
  } else if(i->streams.rightmost != &stream->node || stream->groups.rightmost != &group->node) {
  ...
  if(group != NULL) {
    ...
    iter->block.compressed_stream_offset =
        record == 0 ? group->node.compressed_base : vli_ceil4(group->records[record - 1].unpadded_sum);
    iter->block.uncompressed_stream_offset =
        record == 0 ? group->node.uncompressed_base : group->records[record - 1].uncompressed_sum;
...
extern LZMA_API(void) lzma_index_iter_init(lzma_index_iter* iter, const lzma_index* i) {
...
extern LZMA_API(lzma_bool) lzma_index_iter_next(lzma_index_iter* iter, lzma_index_iter_mode mode) {
    ...
    return true;
  {
    ...
    if(mode != LZMA_INDEX_ITER_STREAM) {
      ...
      switch(iter->internal[ITER_METHOD].s) {
        case ITER_METHOD_NORMAL: group = iter->internal[ITER_GROUP].p; break;

        case ITER_METHOD_NEXT: group = index_tree_next(iter->internal[ITER_GROUP].p); break;

        case ITER_METHOD_LEFTMOST: group = (const index_group*)(stream->groups.leftmost); break;
      }
    }
...
extern LZMA_API(lzma_bool) lzma_index_iter_locate(lzma_index_iter* iter, lzma_vli target) {
  const lzma_index* i = iter->internal[ITER_INDEX].p;
