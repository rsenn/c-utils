...
index_decode(void* coder_ptr,
  ...
  while(*in_pos < in_size) switch(coder->sequence) {
      case SEQ_INDICATOR:
      ...
      case SEQ_MEMUSAGE:
        ...
        ret = LZMA_OK;
        coder->sequence = coder->count == 0 ? SEQ_PADDING_INIT : SEQ_UNPADDED;
        break;

      case SEQ_UNPADDED:
      case SEQ_UNCOMPRESSED: {
        lzma_vli* size = coder->sequence == SEQ_UNPADDED ? &coder->unpadded_size : &coder->uncompressed_size;
...
static lzma_ret
index_decoder_memconfig(void* coder_ptr, uint64_t* memusage, uint64_t* old_memlimit, uint64_t new_memlimit) {
  lzma_index_coder* coder = coder_ptr;
...
static lzma_ret
index_decoder_reset(lzma_index_coder* coder, const lzma_allocator* allocator, lzma_index** i, uint64_t memlimit) {
...
static lzma_ret
index_decoder_init(lzma_next_coder* next, const lzma_allocator* allocator, lzma_index** i, uint64_t memlimit) {
...
extern LZMA_API(lzma_ret) lzma_index_decoder(lzma_stream* strm, lzma_index** i, uint64_t memlimit) {
  lzma_next_strm_init_2(index_decoder_init, strm, i, memlimit);
...
extern LZMA_API(lzma_ret) lzma_index_buffer_decode(lzma_index** i,
  ...
  ret = index_decode(&coder, allocator, in, in_pos, in_size, NULL, NULL, 0, LZMA_RUN);
