...
index_encode(void* coder_ptr,
  ...
  while(*out_pos < out_size) switch(coder->sequence) {
      ...
      case SEQ_UNCOMPRESSED: {
        const lzma_vli size =
            coder->sequence == SEQ_UNPADDED ? coder->iter.block.unpadded_size : coder->iter.block.uncompressed_size;
...
extern lzma_ret
lzma_index_encoder_init(lzma_next_coder* next, const lzma_allocator* allocator, const lzma_index* i) {
...
extern LZMA_API(lzma_ret) lzma_index_encoder(lzma_stream* strm, const lzma_index* i) {
