...
static uint32_t
get_literal_price(const lzma_lzma1_encoder* const coder,
                  const uint32_t pos,
  ...
  const probability* const subcoder =
      literal_subcoder(coder->literal, coder->literal_context_bits, coder->literal_pos_mask, pos, prev_byte);
...
get_len_price(const lzma_length_encoder* const lencoder, const uint32_t len, const uint32_t pos_state) {
...
static inline uint32_t
get_short_rep_price(const lzma_lzma1_encoder* const coder, const lzma_lzma_state state, const uint32_t pos_state) {
  return rc_bit_0_price(coder->is_rep0[state]) + rc_bit_0_price(coder->is_rep0_long[state][pos_state]);
}

static inline uint32_t
get_pure_rep_price(const lzma_lzma1_encoder* const coder,
...
static inline uint32_t
get_rep_price(const lzma_lzma1_encoder* const coder,
              const uint32_t rep_index,
              const uint32_t len,
              const lzma_lzma_state state,
...
static inline uint32_t
get_dist_len_price(const lzma_lzma1_encoder* const coder,
                   const uint32_t dist,
                   const uint32_t len,
                   const uint32_t pos_state) {
...
fill_dist_prices(lzma_lzma1_encoder* coder) {
  ...
  for(uint32_t i = DIST_MODEL_START; i < FULL_DISTANCES; ++i) {
    ...
    const uint32_t price = rc_bittree_reverse_price(coder->dist_special + base - dist_slot - 1, footer_bits, i - base);
...
static void
backward(lzma_lzma1_encoder* restrict coder, uint32_t* restrict len_res, uint32_t* restrict back_res, uint32_t cur) {
...
static inline uint32_t
helper1(lzma_lzma1_encoder* restrict coder,
        lzma_mf* restrict mf,
  ...
  coder->opts[1].price =
      ...
      get_literal_price(coder, position, buf[-1], !is_literal_state(coder->state), match_byte, current_byte);
...
static inline uint32_t
helper2(lzma_lzma1_encoder* coder,
        uint32_t* reps,
        const uint8_t* buf,
        uint32_t len_end,
        uint32_t position,
  ...
  const uint32_t cur_and_1_price =
      ...
      get_literal_price(coder, position, buf[-1], !is_literal_state(state), match_byte, current_byte);
  ...
  if(!next_is_literal && match_byte != current_byte) { // speed optimization
    ...
    if(len_test >= 2) {
      ...
      const uint32_t next_rep_match_price = cur_and_1_price + rc_bit_1_price(coder->is_match[state_2][pos_state_next]) +
