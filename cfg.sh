cfg() {
  if type gcc 2>/dev/null >/dev/null && type g++ 2>/dev/null >/dev/null; then
    ...
    : ${build:=`$CC -dumpmachine | sed 's|-pc-|-|g'`}; if [ -z "${host}" -a -z "${builddir}" ]; then
      host=$build
      case "${host}" in
        x86_64-w64-mingw32) host="${host}"
          : ${builddir=build/$host}
          : ${prefix=/mingw64} ;;
        i686-w64-mingw32) host="${host}"
          : ${builddir=build/$host}
          : ${prefix=/mingw32} ;;
        x86_64-pc-*) host="${host}"
          : ${builddir=build/$host}
     ...
     : ${prefix:=/usr/local}; : ${libdir:=$prefix/lib}; [ -d "${libdir}/$host" ] && libdir=$libdir/$host; if [ -e "${TOOLCHAIN}" ]; then
    ...
    esac; case $(uname -o) in
      *) SYSTEM="Unix" ;;
    esac; case "${STATIC}:$TYPE" in
      YES:*|yes:*|y:*|1:*|ON:*|on:*|*:*[Ss]tatic*) set -- "$@"
        -DBUILD_SHARED_LIBS=OFF
        -DENABLE_PIC=OFF ;;
    esac; [ -n "${PKG_CONFIG_PATH}" ] && echo "PKG_CONFIG_PATH=$PKG_CONFIG_PATH" >&2; [ -n "${PKG_CONFIG}" ] && case "${PKG_CONFIG}" in
  */*)  ;;
  *) PKG_CONFIG=$(which "${PKG_CONFIG}") ;;
esac; : ${generator:="CodeLite - Unix Makefiles"}; (mkdir -p $builddir : ${relsrcdir=`realpath --relative-to "${builddir}" .`} : set -x cd "${builddir:-.}" IFS="${IFS} " set -- -Wno-dev -G "${generator}" ${prefix:+-DCMAKE_INSTALL_PREFIX="${prefix}"} ${VERBOSE:+-DCMAKE_VERBOSE_MAKEFILE=${VERBOSE:-OFF}} -DCMAKE_BUILD_TYPE="${TYPE:-Debug}" -DBUILD_SHARED_LIBS=ON ${CC:+-DCMAKE_C_COMPILER="${CC}"} ${CXX:+-DCMAKE_CXX_COMPILER="${CXX}"} ${PKG_CONFIG:+-DPKG_CONFIG_EXECUTABLE="${PKG_CONFIG}"} ${TOOLCHAIN:+-DCMAKE_TOOLCHAIN_FILE="${TOOLCHAIN}"} ${CC:+-DCMAKE_C_COMPILER="${CC}"} ${CXX:+-DCMAKE_CXX_COMPILER="${CXX}"} ${MAKE:+-DCMAKE_MAKE_PROGRAM="${MAKE}"} "$@" $relsrcdir eval "${CMAKE:-cmake} \"\$@\"") 2>&1 | tee "${builddir##*/}.log"
}
...
cfg-msys32() {
  host=i686-pc-msys; cfg-msys "$@"
}

cfg-msys64() {
  host=x86_64-pc-msys; cfg-msys "$@"
}
...
cfg-emscripten() {
  ...
  PKG_CONFIG_PATH=$(set -- /opt/*-wasm/lib/pkgconfig; IFS=":"; echo "$*")
