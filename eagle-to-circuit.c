...
print_name_value(buffer* b, const char* name, const char* value) {
  buffer_putm_internal(b, name, ": ", value ? value : "(null)", NULL);
}
...
build_deviceset(xmlnode* set) {
  ...
  MAP_INSERT(
      devicesets, name, str_len(name) + 1, &d, sizeof(struct deviceset));
}
...
getnodeset(void* n, const char* xpath) {
  return xml_find_all_1(n, (xml_pred_t*)&xml_match_name, xpath);
}
...
clean_pkgname(stralloc* pkgname, const struct package* pkg) {
  ...
  if(pkgname->len > 4 && pkgname->s[0] == '0' && isdigit(pkgname->s[1]) &&
     isdigit(pkgname->s[2]) && isdigit(pkgname->s[3]) &&
     pkgname->s[4] == '/') {
...
print_element_name(xmlnode* a_node) {
  ...
  if(str_diff(name, "eagle") && str_diff(name, "drawing")) {
    buffer_putm_internal(buffer_2, a_node->parent ? "/" : "", name, NULL);
    if(!(name = xml_get_attribute(a_node, "name")))
      return;
    if(str_len(name))
      buffer_putm_internal(buffer_2, "[@name='", name, "']", NULL);
  }
}
...
print_element_content(xmlnode* node) {
  ...
  if((s = xml_content(node))) {
    if(str_isspace(s))
      s = "";
    if(str_len(s))
      buffer_putm_internal(buffer_2, " \"", s, "\"", NULL);
  }
}
