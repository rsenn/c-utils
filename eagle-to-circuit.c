...
//    return NULL;
//  }
//  if(xmlXPathNodeSetIsEmpty(result->nodeset)) {
//    xmlXPathFreeObject(result);
//    buffer_putsflush(buffer_2, "No result\n");
//    return NULL;
//  }
//  buffer_putm(buffer_2, "xpath: ", xpath, ", num nodes: ");
//  buffer_putulong(buffer_2, result && result->nodeset ? xmlXPathNodeSetGetLength(result->nodeset) : 0);
//  buffer_putnlflush(buffer_2);
//  return result;
//}
...
getparts(xmlnode* doc) {
  strlist ret;
  strlist_init(&ret);
  xmlxpathobj* nodes = getnodeset(doc, "//part | //element");
  if(!nodes || !nodes->nodeset)
    return ret;
  for(int i = 0; i < xmlXPathNodeSetGetLength(nodes->nodeset); ++i) {
    xmlnode* node = xmlXPathNodeSetItem(nodes->nodeset, i);
...
tree_topleft(xmlnode* elem, const char* elems, double* x, double* y) {
  ...
  if(node == 0)
    return;
  while(node && node->type != XML_NODE_ELEMENT && str_diff(NODE_NAME(node), elems))
    node = node->next;
  *x = get_double(node, "x");
  *y = get_double(node, "y");
...
xpath_query(xmlnode* doc, const char* q) {
  ...
  buffer_putnlflush(buffer_1);
  xmlxpathobj* xr = NULL; // = getnodeset(doc, q);
  if(!xr || !xr->nodeset)
    return;
  for(int i = 0; i < xml_nodeset_length(xr->nodeset); ++i) {
    xmlnode* node = xml_nodeset_item(xr->nodeset, i);
    print_element_name(node);
    print_element_attrs(node);
    buffer_putnlflush(buffer_1);
...
xpath_foreach(xmlnode* doc, const char* q, void (*fn)(xmlnode*)) {
  xmlxpathobj* xpo = NULL; //getnodeset(doc, q);
  if(xpo && &xpo->nodeset) {
    //for_set(xpo->nodeset, fn);
    return true;
  }
  return false;
}
