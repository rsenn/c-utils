...
#define ELF_DUMP_FIELD(base, ptr, st, field)                                                                                                                                                           \
  buffer_putspad(b, #field, 30), buffer_puts(b, " 0x"), buffer_putxlonglong0(b, ELF_GET(base, ptr, st, field), ELF_SIZE(base, st, field) * 2), buffer_putnlflush(b)
...
main(int argc, char** argv) {
  ...
  struct longopt opts[] = {{"help", 0, NULL, 'h'}, {"defined", 0, &list_defined, 'D'}, {"undefined", 0, &list_undefined, 'U'}, {"file-header", 0, 0, 'F'}, {0, 0, 0, 0}};
...
elf_dump_segments(range map) {
  ...
  range_foreach(&segments, segment) {
    ...
    buffer_putm_internal(buffer_1, " ", (flags & ELF_PF_R) ? "r" : "-", (flags & ELF_PF_W) ? "w" : "-", (flags & ELF_PF_W) ? "x" : "-", 0);
    buffer_putnlflush(buffer_1);
  }
}
