...
print_shdr64(elf64_shdr* shdr) {
  ...
  buffer_puts(buffer_1, " '");
  buffer_putspad(buffer_1, &elf_shstrtab(ehdr)[shdr->sh_name], 16);
  buffer_puts(buffer_1, "', flags=");
...
process64(elf64_ehdr* hdr) {
  ...
  for(i = 0; i < hdr->e_shnum; ++i) {
    const char* name = elf_shstrtab(hdr) + shdrs[i].sh_name;
...
elfwrsec(const char* file) {
  ...
  ret =
      (header->e_ident[ELF_EI_CLASS] == ELF_ELFCLASS64 ? process64(base)
                                                       : process32(base));
