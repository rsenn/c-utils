...
blockingcopy(int src, int dest) {
  ...
  for(;;) {
    ...
    if(i == -1)
      panic("server: error: read() failed");
    ...
    for(j = 0; j < i;) {
      ...
      if(k == -1)
        panic("server: error: write() failed");

      if(k == 0)
        panic("server: error: short write\n");
      j += k;
    }
  }
}
...
main(int argc, char* argv[]) {
  ...
  if(!strcmp(argv[1], "0"))
    byte_zero(ip, 16);
  else if(argv[1][i = scan_ip6if(argv[1], ip, &scope_id)]) {
  ...
  if(byte_equal(ip, 12, V4mappedprefix)) {
    ...
    if(s == -1)
      panic("server: error: socket() failed");

    if(socket_bind4_reuse(s, ip + 12, port) == -1)
      panic("server: error: bind() failed");

    if(socket_listen(s, 1) == -1)
      panic("server: error: listen() failed");
    ndelay_off(s);

    if((t = socket_accept4(s, 0, 0)) == -1)
      panic("server: error: accept() failed");
  } else {
    s = socket_tcp6b();
