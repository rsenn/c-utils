  ...
  for(pos = 1; pos < len; pos++) {
    ...
    switch(buf[start]) {
      ...
      case 'm':
        if(pos - start - 1 == 0)
          return 0;
        if(get_ulong(buf + start + 1, pos - start - 1, &ul) !=
           pos - start - 1)
          return 0;
        tai_unix(&f->mtime, ul);
    ...
    switch(state) {
      case 0: /* month */
        if('-' == buf[pos]) {
          ...
          if(pos == start)
            return 0;
          if(get_ulong(buf + start, pos - start, &mon) != pos - start)
            return 0;
          start = pos + 1;
        }
        break;
      case 1: /* day */
        if('-' == buf[pos]) {
          ...
          if(pos == start)
            return 0;
          if(get_ulong(buf + start, pos - start, &day) != pos - start)
            return 0;
          start = pos + 1;
        }
        break;
      ...
      case 4: /* time */
        x = scan_time(
            buf + start, len - pos, &hour, &min, &sec, &mtimetype);
        if(!x)
          return 0;
        pos += x;
        if(pos == len)
      ...
      case 6: /* <DIR> or length */
        if(' ' == buf[pos]) {
          if(get_uint64(buf + start, pos - start, &size) != pos - start) {
            if(pos - start < 5 || !my_byte_equal(buf + start, 5, "<DIR>"))
              return 0;
            flagtrycwd = 1;
            maxspaces = 10;
          } else {
...
static int
parse_multinet(struct ftpparse* f,
               char* p[],
               int l[],
               unsigned int count) {
...
parse_unix(struct ftpparse* f,
  ...
  if('l' == *buf) {
    ...
    for(j = 1; j < f->namelen - 4; j++) /* 1, -4: no empty names,
                                           please */
      if(f->name[j] == ' ' && f->name[j + 1] == '-' &&
         f->name[j + 2] == '>' && f->name[j + 3] == ' ') {
...
static int
parse_supertcp(struct ftpparse* f,
               char* p[],
               int l[],
               unsigned int count) {
  unsigned long mon;
...
ftpparse_int(struct ftpparse* fp, char* buf, int len) {
  ...
  switch(*buf) {
    ...
    case 's':
    case '-':
      if(parse_unix(fp, buf, len, p, l, count))
        return 1;
      break;
  }
