...
/* a replacement for daniel bernsteins
 * ftpparse library. */

/*
 * ftpparse(&fp,buf,len) tries to parse
 one line of LIST output.
 *
 * The line is an array of len
 characters stored in buf.
 * It should not include the terminating
 CR LF; so buf[len] is typically CR.
 *
 * If ftpparse() can't find a filename,
 it returns 0.

 * If ftpparse() can find a filename, it
 fills in fp and returns 1.
 * fp is a struct ftpparse, defined
 below.
 * The name is an array of fp.namelen
 characters stored in fp.name;
 * fp.name points somewhere within buf.
 */

/* differences between Bernsteins and
 * this version: a) use tai, not time_t.
 * b) included format flag.
 * c) size is now an uint64 (was signed
 * long). d) added
 * FTPPARSE_MTIME_REMOTESECOND: time
 * zone is unknown, seconds known e)
 * much more formats understood. f) much
 * more picky about some details. g)
 * there's a function to parse MLSX.
 */
#include "lib/tai.h"
#include "lib/uint64.h"

struct ftpparse {
  char* name; /* not necessarily
                 0-terminated */
  unsigned int namelen;
  int flagtrycwd;  /* 0 if cwd is
                      definitely
                      pointless, 1
                      otherwise */
  int flagtryretr; /* 0 if retr is
                      definitely
                      pointless, 1
                      otherwise */
  int sizetype;
  ...
  int idtype;
  char* id; /* not necessarily
               0-terminated */
  unsigned int idlen;
  int format;
  int flagbrokenmlsx;
  char* symlink; /* 0: no symlink. !=0:
                    offset to start of
                    it */
...
/*
 * When a time zone is unknown, it is
 * assumed to be GMT. You may want to
 * use localtime() for LOCAL times,
 * along with an indication that the
 * time is correct in the local time
 * zone, and gmtime() for REMOTE* times.
 */
