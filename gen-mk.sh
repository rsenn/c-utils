...
for compiler; do 
  for build_type in Debug RelWithDebInfo MinSizeRel Release; do
	...
	mkdir -p $output_dir
 (exec_cmd "genmakefile --create-bins --create-libs -n shish -m ninja -t $compiler --$build_type lib/{\$MODULES}/*.c \$PROGRAMS -o $output_dir/build.ninja"
  exec_cmd "genmakefile --create-bins --create-libs -n shish -m make -t $compiler --$build_type lib/{\$MODULES}/*.c \$PROGRAMS -o $output_dir/Makefile")
  done
done
