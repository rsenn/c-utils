...
target*     find_rule_b(const char* x, size_t n);
target*     find_rule(const char* needle);
target*     find_rule_sa(stralloc* name);
...
int         gen_link_rules(HMAP_DB* rules, strlist* sources);
target*     gen_simple_compile_rules(HMAP_DB* rules, sourcedir* srcdir, const char* dir, const char* fromext, const char* toext, stralloc* cmd);
...
target*     get_rule(const char* name);
target*     get_rule_sa(stralloc* name);
...
int         match_rule(target* rule, const char* pattern);
