...
buffer_brotli_close(buffer* b) {
  ...
  if(b->op == (buffer_op_proto*)&buffer_brotli_write) {
    ...
    do {
      ...
      ret =
          BrotliDecoderDecompressStream(ctx->state, &avail_in, &next_in, &avail_out, &next_out, 0);
    } while(ret != BROTLI_FALSE && avail_in > 0);
...
buffer_brotli(buffer* b, buffer* other, int compress) {
  ...
  buffer_init(
      b, (buffer_op_proto*)(compress ? &buffer_brotli_write : &buffer_brotli_read), -1, buf, 1024);
  b->cookie = ctx;
