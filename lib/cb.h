...
int cb_find_prefix(const critbit_tree* cb,
                   const void* key,
                   size_t keylen,
                   void** results,
                   int numresults,
                   int offset);
int cb_foreach(critbit_tree* cb,
               const void* key,
               size_t keylen,
               int (*match_cb)(const void* match,
                               const void* key,
                               size_t keylen,
                               void*),
               void* data);
...
#define cb_insert_str(cb, key) cb_insert(cb, (void*)key, str_len(key) + 1)
#define cb_find_str(cb, key) cb_find(cb, (void*)key, str_len(key) + 1)
#define cb_erase_str(cb, key) cb_erase(cb, (void*)key, str_len(key) + 1)
#define cb_find_prefix_str(cb, key, results, numresults, offset) \
  cb_find_prefix( \
      cb, (const void*)key, str_len(key), results, numresults, offset)

#define CB_KV_SIZE(keylen, datalen) (keylen + datalen + 1)
size_t cb_new_kv(const char* key,
                 size_t keylen,
                 const void* value,
                 size_t len,
                 void* out);
