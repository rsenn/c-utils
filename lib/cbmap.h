...
typedef int (*cbmap_visitor)(const void* key,
                             size_t key_len,
                             const void* value,
                             size_t value_len,
                             void* user_data);
...
/**
 ...
 Keys and values may be changed during the map lifetime, because copies are not
 modified.
*/
cbmap_t cbmap_new(void);

/**
 Returns a map that does not allocate nor deallocate keys (just keeps pointers
 to them). WARNING: Of course, expect undefined behaviour if keys and values are
