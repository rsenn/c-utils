...
cbmap_data_node_update_value(cbmap_allocator_t value_allocator,
  ...
  if(data->value_len == value_len &&
     VALUE_COMPARE(data->value, value, value_len) == 0) {
    return data->value;
  }
  ...
  data->value = (unsigned char*)new_value;
...
cbmap_data_node_new(cbmap_allocator_t key_allocator,
  ...
  /* Allocate value */
  data->value_len = value_len;
  data->value = (unsigned char*)value_allocator->malloc(value, value_len);
  if(data->value == NULL) {
    CBM_FREE(data);
    return NULL;
  }
  ...
  data->key = (unsigned char*)key_allocator->malloc(key, key_len);
  if(data->key == NULL) {
    value_allocator->free(data->value);
    CBM_FREE(data);
    return NULL;
...
int
cbmap_insert(
    cbmap_t map, void* key, size_t key_len, void* value, size_t value_len) {
  unsigned char c, *p = (unsigned char*)map->root;
  ...
  if(p == NULL) {
    struct cbmap_data_node* data = cbmap_data_node_new(&map->key_allocator,
                                                       &map->value_allocator,
                                                       key,
                                                       key_len,
                                                       value,
  ...
  while(IS_INTERNAL_NODE(p)) {
    ...
    p = (unsigned char*)node->branch[direction];
  }
  ...
  if(newbyte == key_len) {
    void* result = cbmap_data_node_update_value(&map->value_allocator,
                                                data,
                                                value,
                                                value_len);
...
different_byte_found:
  ...
  newdata = cbmap_data_node_new(&map->key_allocator,
                                &map->value_allocator,
                                key,
                                key_len,
                                value,
  ...
  for(;;) {
    unsigned char c = 0;
    unsigned char* p = (unsigned char*)*insertion_node;
    int direction;
