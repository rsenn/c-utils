...
void dlist_init(list*);
size_t dlist_length(list*);
void dlist_push(list*, node*);
void dlist_remove(list*, node*);
void dlist_unshift(list*, node*);
...
#define dlist_foreach_up(list, n) for((n) = (void*)(list)->tail; (n) != NULL; (n) = (void*)(((node*)n)->prev))

/* n is set to the current node and n->prev is backupped
   into m before loop body for safe walk-throught when
   links get deleted */
#define dlist_foreach_up_safe(list, n, m)                                                                                                                                                               \
