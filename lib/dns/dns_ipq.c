...
static int
doit(stralloc* work, const char* rule) {
  char ch;
  unsigned int colon;
...
int
dns_ip4_qualify_rules(stralloc* out, stralloc* fqdn, const stralloc* in, const stralloc* rules) {
  unsigned int i;
  unsigned int j;
  unsigned int plus;
  ...
  if(plus >= fqdnlen) return dns_ip4(out, fqdn);
...
int
dns_ip4_qualify(stralloc* out, stralloc* fqdn, const stralloc* in) {
