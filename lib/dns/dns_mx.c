...
int
dns_mx_packet(stralloc* out, const char* buf, unsigned int len) {
  unsigned int pos;
  char header[12];
  char pref[2];
  uint16 numanswers;
  ...
  while(numanswers--) {
    ...
    if(byte_equal(header, 2, DNS_T_MX))
      if(byte_equal(header + 2, 2, DNS_C_IN)) {
        if(!dns_packet_copy(buf, len, pos, pref, 2))
          return -1;
        if(!dns_packet_getname(buf, len, pos + 2, &q))
          return -1;
        if(!stralloc_catb(out, pref, 2))
          return -1;
...
int
dns_mx(stralloc* out, const stralloc* fqdn) {
