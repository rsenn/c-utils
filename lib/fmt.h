...
#define FMT_ULONG \
  40 /* enough space to hold 2^128 - 1 in decimal, plus \0 */
#define FMT_8LONG \
  44 /* enough space to hold 2^128 - 1 in octal, plus \0 \
      */
#define FMT_XLONG \
...
size_t fmt_asn1derlength(
    char* dest,
    uint64 l); /* 0 - 0x7f: 1 byte, above that 1 + bytes_needed bytes */
