...
gpio_init() {
  ...
  if(gpio != NULL) {
    buffer_puts(buffer_1,
                "gpio_init : Error: Seems to be already initialized!");
  ...
  mem_map =
      mmap(NULL, /* Local mapping start address (NULL means don't care). */
  ...
  if(mem_map == MAP_FAILED) {
    buffer_puts(buffer_1,
                "gpio_init : Error: Failed to create memory mapping!");
    buffer_putnlflush(buffer_1);
    return 0;
  }
#endif
