...
int hmap_add_tuple_with_data(HMAP_DB** hmap_db,
                             const void* key,
                             size_t k_len,
                             void* data);
int hmap_add(HMAP_DB** hmap_db,
             const void* key,
             size_t k_len,
             int dup_flag,
             int data_type,
...
TUPLE*
hmap_set(HMAP_DB**, const void*, size_t k_len, void* data, size_t d_len);
...
#define hmap_next(hmap_db, tuple) \
  (hmap_last(hmap_db, tuple) ? NULL : (tuple)->next)

#define hmap_foreach(hmap_db, tuple) \
  for(tuple = hmap_begin(hmap_db); tuple; \
      tuple = hmap_next(hmap_db, tuple))
