...
http_ssl_ctx(void) {
  ...
  if(!SSL_CTX_use_RSAPrivateKey_file(ctx, "../c-utils/http.key", SSL_FILETYPE_PEM) || !SSL_CTX_use_certificate_file(ctx, "../c-utils/http.crt", SSL_FILETYPE_PEM) || !SSL_CTX_check_private_key(ctx)) {
