...
void
hex_data::set(address_type address, value_type value) {
...
void
hex_data::erase(address_type address) {
...
/* Erase [first, last] */
void
hex_data::erase(address_type first, address_type last) {
  if(first > last)
    std::swap(first, last);
...
hex_data::size_type
hex_data::size_below_addr(address_type addr) {
  size_type s = 0;
...
hex_data::size_type
hex_data::size_in_range(address_type lo, address_type hi) {
  size_type s = 0;
...
/* Lowest address */
address_type
hex_data::min_address() const {
  return blocks.begin()->first;
}

/* Highest address */
address_type
hex_data::max_address() const {
...
/* Load from a file */
void
hex_data::load(const std::string& path) {
...
uint8_t
hex2binary(hex_data::data_container& to, std::string& from) {
...
void
hex_data::write(const char* path) {
...
void
hex_data::tidy(hex_data::size_type length) {
  for(iterator i = blocks.begin(); i != blocks.end(); i++) {
