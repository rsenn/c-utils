...
iob_send(int64 s, io_batch* b) {
  ...
  if(x[i].type == FROMBUF) {
    ...
    if(x + i + 1 < last && (x[i + 1].type == FROMFILE)) {
    ...
    e[i].os.Offset = x[i].offset;
    e[i].os.OffsetHigh = (x[i].offset >> 32);
    if(!TransmitFile(s, (HANDLE)x[i].fd, x[i].n > 0xffff ? 0xffff : x[i].n, 0, &e[i].os, 0, TF_USE_KERNEL_APC))
      return -3;
    return sent;
  }
}
...
iob_send(int64 s, io_batch* b) {
  ...
  for(;;) {
    ...
    if(e[i].type == FROMFILE) {
      ...
      hdr.headers = v;
      hdr.hdr_cnt = headers;
      hdr.trailers = v + headers;
      hdr.trl_cnt = trailers;
