...
#define MAP_FOREACH(map, iter) for(linked_list_node* node = linked_list_head(hashmap_keys(&(map))); (iter = node ? hashmap_getpair(hashmap_search(&(map), node->data)) : 0); node = node->next)
