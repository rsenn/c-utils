...
#define MAP_FOREACH(map, pair) for(MAP_ITER_T iter = linked_list_head(hashmap_keys(&(map))); iter && ((pair) = hashmap_find(&(map), iter->data)); (iter) = (iter)->next)
#define MAP_FOREACH_SAFE(map, pair) \
  for(MAP_ITER_T next, iter = linked_list_head(hashmap_keys(&(map))); iter && ((next = (iter)->next), ((pair) = hashmap_find(&(map), iter->data))); (iter) = next)
