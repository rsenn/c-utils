...
#define MAP_FOREACH_VALUE(map, pair, value) for(MAP_ITER_T iter = linked_list_head(hashmap_keys(&(map))); iter && (((pair) = hashmap_find(&(map), iter->data)), ((value) = MAP_VALUE((pair)))); (iter) = (iter)->next)
