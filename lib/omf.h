...
#define OMF_LNAME_LAST                                                         \
  0x05 // Last default name. Nondefault names start at OMF_LNAME_LAST + 1
#define OMF_THEADR 0x80 // Translator Header Record
#define OMF_LHEADR 0x82 // Library Module Header Record
#define OMF_COMENT                                                             \
  0x88 // Comment Record (Including all comment class extensions)
...
#define omf_foreach(omf, r)                                                    \
  for((r) = omf_begin(omf);                                                    \
      (char*)r + 2 + uint16_get(&(r)->length) < (char*)omf_end(omf);           \
      (r) = omf_next(r))

#define omf_record_foreach(rec, ptr)                                           \
  for((ptr) = omf_record_begin(rec);                                           \
      (char*)(ptr) < (char*)omf_record_end(rec);                               \
