...
path_gethome(int uid) {
  ...
  if(buffer_mmapread(&b, "/etc/passwd")) return NULL;

  for(;;) {
    /* skip the next two colon-separators to get the uid */
    if(buffer_skip_until(&b, ":", 1) <= 0) break;
    if(buffer_skip_until(&b, ":", 1) <= 0) break;

    /* get the uid or the home */
    if((n = buffer_get_until(&b, home, PATH_MAX + 1, ":", 1)) <= 0) break;
    ...
    /* parse the uid */
    if(!scan_ulong(home, (unsigned long*)&id)) id = -1;
