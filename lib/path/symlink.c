...
CreateSymlink(LPCTSTR lpLinkName,
...

  {
    ...
    MultiByteToWideChar(
        CP_ACP, 0, namebuf, -1, namebuf_w, MAXIMUM_REPARSE_DATA_BUFFER_SIZE);
#endif
    ...
    u.iobuf.ReparseDataLength =
        12 + u.iobuf.SymbolicLinkReparseBuffer.SubstituteNameOffset +
        u.iobuf.SymbolicLinkReparseBuffer.SubstituteNameLength;
    cb = 8 + u.iobuf.ReparseDataLength;

    if(!DeviceIoControl(
           hFile, FSCTL_SET_REPARSE_POINT, &u.iobuf, cb, NULL, 0, &cb, NULL)) {
...
CreateJunction(LPCTSTR lpLinkName,
  ...
  byte_zero((char*)u.iobuf.MountPointReparseBuffer.PathBuffer +
            ...
            4);
  u.iobuf.ReparseDataLength =
      8 + u.iobuf.MountPointReparseBuffer.PrintNameOffset +
