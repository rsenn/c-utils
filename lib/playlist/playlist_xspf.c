...
xspf_process(playlist* pl) {
  ...
  if(value || xmlTextReaderReadString(reader)) {
    const char* s = value ? (const char*)value
                          : (const char*)xmlTextReaderReadString(reader);
    while(*s && (isspace(*s) || *s == '\n' || *s == '\r' || *s == '\t' ||
                 *s == ' ')) {
      s++;
    }
  ...
  /*  if(name[0] != '#') {
  //   printf("%d: <%s%s(%d))>%s\n", xmlTextReaderDepth(reader), nodeType == 15
  ? "/" : "", name, nodeType, nodeText.s);
    }*/
  if(name[0] != '#') { stralloc_zero(&nodeText); }
}
...
playlist_xspf(playlist* pls, buffer* b) {
  xmlTextReaderPtr rd;
  rd = xmlReaderForIO((xmlInputReadCallback)buffer_get,
                      (xmlInputCloseCallback)buffer_close_return_int,
                      b,
                      "",
                      "UTF-8",
                      0);
