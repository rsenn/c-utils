...
range_ptrinbuf(const void* buf, size_t len, const void* ptr) {
  ...
  return __likely(
      ...
      (size_t)((const char*)ptr - c) < len); /* this one is a little tricky.
         "ptr-c" checks the offset of ptr in the buffer is inside the buffer
         size. Now, ptr-c can underflow; say it is -1.  When we cast it to
         size_t, it becomes a very large number. */
}
...
#define add_of(result, type, c, a, b)                                          \
  {                                                                            \
    ...
    result =                                                                   \
        (__b) < 1                                                              \
            ? ((__MIN(type) - (__b) <= (__a)) ? assign(type, c, __a + __b)     \
                                              : 1)                             \
            : ((__MAX(type) - (__b) >= (__a)) ? assign(type, c, __a + __b)     \
                                              : 1);                            \
  }
...
#define sub_of(c, a, b)                                                        \
  {                                                                            \
    ...
    typeof(b) __b = b;                                                         \
    (__b) < 1                                                                  \
        ? ((__MAX(typeof(a + b)) + __b >= __a) ? assign(c, __a - __b) : 1)     \
