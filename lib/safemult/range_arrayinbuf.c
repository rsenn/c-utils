...
int
range_arrayinbuf(
    const void* buf1, size_t len, const void* arraystart, size_t elements, size_t membersize) {
  size_t alen;
  if(sizeof(alen) == 8) {
    uint64 x;
    if(!umult64(elements, membersize, &x))
      return 0;
    alen = x;
  } else {
    ...
    if(alen != t)
      return 0; /* if that changes something, we overflowed */
  }
