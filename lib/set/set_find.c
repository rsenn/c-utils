...
bucket_t**
set_find_bucket(bucket_t**ptr, const void* val, const size_t size, uint32 hash) {bucket_t* b;
  while((b = *ptr)) {
...
set_find(const set_t* set, const void* val, const size_t size) {
 bucket_t** ptr; uint32 hash = set_hash(set, val, size);
  set_iterator_t it;
  it.set = set;
  it.index = hash % set->len;
it.current = &set->array[it.index];

return set_find_bucket(&it.current,val,size,hash);
/*
