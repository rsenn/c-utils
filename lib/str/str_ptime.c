...
str_ptime(const char* s, const char* format, struct tm* tm) {
      ...
      case '%':
        ...
        switch(*format) {
          ...
          case 'h':
            for(i = 0; i < 12; ++i) {
              if(str_case_diffn(s, months[i], j = str_len(months[i])))
                if(str_case_diffn(s, months[i], j = 3))
                  j = 0;
              if(j) {
                break;
              };
            ...
            tm->tm_mon = i;
            break;
          case 'c': s = str_ptime(s, "%b %a %d %k:%M:%S %Z %Y", tm); break;
          case 'C':
            i = get_int(&s, 2);
            if(i == -1)
              return 0;
          ...
          case 'e':
            ...
            tm->tm_mday = i;
            break;
          case 'D': s = str_ptime(s, "%m/%d/%y", tm); break;
          case 'H':
          case 'k':
            i = get_int(&s, 2);
            if(i == -1 || i > 23)
          ...
          case 'P':
            ...
            s += 2;
            break;
          case 'r': s = str_ptime(s, "%I:%M:%S %p", tm); break;
          case 'R': s = str_ptime(s, "%H:%M", tm); break;
          case 'S':
            i = get_int(&s, 2);
            if(i == -1 || i > 60)
              return 0;
            tm->tm_sec = i;
            break;
          case 'T': s = str_ptime(s, "%H:%M:%S", tm); break;
          case 'U':
          case 'W':
            if(get_int(&s, 2) == -1)
              return 0;
          ...
          case 'w':
            ...
            ++s;
            break;
          case 'x': s = str_ptime(s, "%b %a %d", tm); break;
          case 'X': s = str_ptime(s, "%k:%M:%S", tm); break;
          case 'y':
            i = get_int(&s, 2);
            if(i < 0)
              return 0;
  ...
  day += tm->tm_yday =
      __spm[tm->tm_mon] + tm->tm_mday - 1 + (isleap(tm->tm_year + 1900) & (tm->tm_mon > 1));
  tm->tm_wday = (day + 4) % 7;
