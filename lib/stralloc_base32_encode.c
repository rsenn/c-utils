...
static unsigned int encode(stralloc *out, register uint64_t v, unsigned int octets)
{
  ...
  data[0] = char_base32_chars[ch1];  /* 1 */
  data[1] = char_base32_chars[ch2];  /* 2 */

  if (octets == 1) {  /* 8 bits remaining */
    return stralloc_catb(out, data, 8);
  }

  data[2] = char_base32_chars[ch3];  /* 3 */
  data[3] = char_base32_chars[ch4];  /* 4 */

  if (octets == 2) {  /* 16 bits remaining */
    return stralloc_catb(out, data, 8);
  }

  data[4] = char_base32_chars[ch5];  /* 5 */

  if (octets == 3) {   /* 24 bits remaining */
    return stralloc_catb(out, data, 8);
  }

  data[5] = char_base32_chars[ch6];  /* 6 */
  data[6] = char_base32_chars[ch7];  /* 7 */

  if (octets == 4) {   /* 32 bits remaining */
    return stralloc_catb(out, data, 8);
  }

  data[7] = char_base32_chars[ch8];  /* 8 */

  return stralloc_catb(out, data, 8);
}

unsigned int stralloc_base32_encode(stralloc *out, const stralloc *in)
{
