...
static unsigned int encode(stralloc *out, register uint32_t v, unsigned int octets)
{
  ...
  data[0] = char_base64_chars[ch1];  /* 1 */
  data[1] = char_base64_chars[ch2];  /* 2 */

  if (octets == 1) {  /* 8 bits remaining */
    return stralloc_catb(out, data, 4);
  }

  data[2] = char_base64_chars[ch3];  /* 3 */

  if (octets == 2) {  /* 16 bits remaining */
    return stralloc_catb(out, data, 4);
  }

  data[3] = char_base64_chars[ch4];  /* 4 */

  return stralloc_catb(out, data, 4);
}

unsigned int stralloc_base64_encode(stralloc *out, const stralloc *in)
{
