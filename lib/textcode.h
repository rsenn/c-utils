...
/* Worst case: len*3 */
size_t fmt_quotedprintable2(char* dest,
                            const char* src,
                            size_t len,
                            const char* escapeme);
/* Worst case: len*3 */
...
/* Worst case: len*3 */
size_t fmt_urlencoded2(char* dest,
                       const char* src,
                       size_t len,
                       const char* escapeme);
/* Worst case: len*2 */
...
/* Change '<' to '&lt;' and '&' to '&amp;' and '"' to '&quot;'; worst case:
 * len*6 */
/* This is meant for outputting text that goes in a tag argument between
 * double quotes*/
...
size_t fmt_ldapescape2(char* dest,
                       const char* src,
                       size_t len,
                       const char* escapeme);
 ...
 * should be able to hold str_len(src) bytes as a rule of thumb. */
...
#define fmt_uuencoded_sa(sa, src, len) \
  fmt_to_sa(fmt_uuencoded, sa, src, len)
#define fmt_base64_sa(sa, src, len) fmt_to_sa(fmt_base64, sa, src, len)
#define fmt_base64url_sa(sa, src, len) \
  fmt_to_sa(fmt_base64url, sa, src, len)
#define fmt_quotedprintable_sa(sa, src, len) \
  fmt_to_sa(fmt_quotedprintable, sa, src, len)
#define fmt_urlencoded_sa(sa, src, len) \
  fmt_to_sa(fmt_urlencoded, sa, src, len)
...
#define fmt_ldapescape_sa(sa, src, len) \
  fmt_to_sa(fmt_ldapescape, sa, src, len)
#define fmt_jsonescape_sa(sa, src, len) \
  fmt_to_sa(fmt_jsonescape, sa, src, len)
...
#define fmt_cescape2_sa(sa, src, len, escapeme) \
  fmt_to_sa2(fmt_cescape2, sa, src, len, escapeme)
...
#define scan_quotedprintable_sa(src, sa) \
  scan_to_sa(scan_quotedprintable, src, sa)
...
void
fmt_tofrom_array2(size_t (*func)(char*, const char*, size_t, const char*),
                  array* dest,
                  array* src,
                  const char* escapeme);
...
extern const char base64[66];
extern const char base64url[66];
