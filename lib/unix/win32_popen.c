...
FILE* __cdecl popen(const char* cmdstring, const char* type) {
  ...
  __try {
    ...
    (lstrcatn(CommandLine, _T(" /c "), CommandLineSize));
    (lstrcatn(CommandLine, (char*)cmdstring, CommandLineSize));
...
static IDpair* __cdecl idtab(FILE* pstream) {
  ...
  if((pstream != NULL) || ((__idtabsiz + 1) < __idtabsiz) ||
     ...
     ((newptr = (IDpair*)_recalloc_crt((void*)__idpairs, (__idtabsiz + 1), sizeof(IDpair))) ==
      NULL))
