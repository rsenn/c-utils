...
void *MapPointer (soff_entry *soffs, int soffs_len, uint32 in_ptr, int *section)
{
  int i;
...
/*
int FindSectionID (IMAGE_OPTIONAL_HEADER *oh, uint32 address, uint32 size)
{
  int i;
...
int FindSectionByRawData (LOADED_IMAGE *img, uint32 address)
{
  ULONG i;
  for (i = 0; i < img->NumberOfSections; i++)
  {
    uint32 start = img->Sections[i].VirtualAddress;
    uint32 end = start + img->Sections[i].SizeOfRawData;
...
struct DepTreeElement *ProcessDep (BuildTreeConfig* cfg, soff_entry *soffs, int soffs_len, uint32 name, struct DepTreeElement *root, struct DepTreeElement *self, int deep)
{
...
/*
struct ExportTableItem *FindExportForward (struct DepTreeElement *self, char *dllname, char *export_name, uint32 export_ordinal)
{
  return NULL;
}
*/
...
static uint64_t thunk_data_u1_function (void *thunk_array, uint32 index, BuildTreeConfig *cfg)
{
...
static void *opt_header_get_dd_entry (void *opt_header, uint32 entry_type, BuildTreeConfig *cfg)
{
  if (cfg->machineType == IMAGE_FILE_MACHINE_I386)
...
static void BuildDepTree32or64 (LOADED_IMAGE *img, BuildTreeConfig* cfg, struct DepTreeElement *root, struct DepTreeElement *self, soff_entry *soffs, int soffs_len)
{
  ...
  if (idata->Size > 0 && idata->VirtualAddress != 0)
  {
    ...
    if (ied && ied->NumberOfFunctions > 0)
    {
      ...
      addrs = (uint32 *) MapPointer (soffs, soffs_len, ied->AddressOfFunctions, NULL);
      ords = (WORD *) MapPointer (soffs, soffs_len, ied->AddressOfNameOrdinals, NULL);
      names = (uint32 *) MapPointer (soffs, soffs_len, ied->AddressOfNames, NULL);
