  ...
  for(p = src, n = 0; *p && n < len - 1; p++, n++)
    ...
    if(*p == '\\')
      ...
      switch(*++p) {
        ...
        case 'v': *dst++ = '\v'; break;
        case '0': *dst++ = '\0'; break;
        default: syslog(LOG_ERR, "invalid escape in %s", src); exit(1);
      }
    else
      *dst++ = *p;
  *dst = 0;
...
uu_lockerr(int e) {
  switch(e) {
    case UU_LOCK_INUSE: return "Device is in use";
    case UU_LOCK_OPEN_ERR: return "File open error";
    case UU_LOCK_READ_ERR: return "File read error";
    case UU_LOCK_CREAT_ERR: return "File creation error";
    case UU_LOCK_WRITE_ERR: return "File write error";
    case UU_LOCK_LINK_ERR: return "File link error";
    case UU_LOCK_TRY_ERR: return "File try error";
    case UU_LOCK_OWNER_ERR: return "File owner error";
  }
}
...
main(int argc, char* argv[]) {
  ...
  if((pfd[0].fd = open(devname, O_RDWR | O_NONBLOCK)) < 0) {
    syslog(LOG_ERR,
           "unable to open monitor file %s: %m",
           devname);
    exit(1);
  }
  ...
  for(;;) {
    ...
    switch(lockresult = uu_lock(argv[1])) {
      case UU_LOCK_OK:
        ...
        if(need_init) {
          if(init_index == INIT_ARGV) {
            ...
            if((pfd[0].fd = open(devname, O_RDWR | O_NONBLOCK)) < 0) {
              syslog(LOG_ERR,
                     "unable to re-open monitor file %s: %m",
                     devname);
              exit(1);
            }
      ...
      case UU_LOCK_INUSE:
        ...
        break;
      default: syslog(LOG_ERR, "uu_lock error %s", uu_lockerr(lockresult));
    }
  }
}
