...
resize_array(void** data, uint64* data_size, size_t sizeof_data) {
  ...
  byte_zero(((unsigned char*)new_data) + (*data_size * sizeof_data),
            (new_size - (*data_size)) * sizeof_data);
  *data = new_data;
  *data_size = new_size;
}

#define resize_dep_list(ptr_deptree, ptr_deptree_size) \
  resize_array((void**)ptr_deptree, \
               ptr_deptree_size, \
               sizeof(struct dep_tree_element*))
#define resize_import_list(ptr_import_list, ptr_import_list_size) \
  resize_array((void**)ptr_import_list, \
               ptr_import_list_size, \
               sizeof(struct import_table_item))
#define resize_stack(ptr_stack, ptr_stack_size) \
  resize_array((void**)ptr_stack, ptr_stack_size, sizeof(char*))
...
int
find_dep(struct dep_tree_element* root,
         char* name,
         struct dep_tree_element** result) {
  int ret = -1;
  uint64 i;
...
process_dep(build_tree_config* cfg,
  ...
  if(dllname == NULL)
    return NULL;
  if(str_len(dllname) > 10 &&
     str_case_diffn("api-ms-win", dllname, 10) == 0) {
  ...
  if(found < 0) {
    child =
        (struct dep_tree_element*)malloc(sizeof(struct dep_tree_element));
...
*find_export_forward (struct
dep_tree_element *self, char *dllname,
char *export_name, uint32
export_ordinal)
{
  return NULL;
}
...
static uint64
thunk_data_u1_function(void* pe,
                       void* thunk_array,
                       uint32 index,
...
/*
static void*
opt_header_get_dd_entry(void*
opt_header, uint32 entry_type,
build_tree_config* cfg) {
  if(cfg->machine_type ==
PE_FILE_MACHINE_I386) return
&(((pe32_opt_header*)opt_header)->data_directory[entry_type]);
  else
    return
&(((pe64_opt_header*)opt_header)->data_directory[entry_type]);
}*/
...
build_dep_tree32or64(pe_loaded_image* img,
  ...
  if(idata->size > 0 && idata->virtual_address != 0) {
    ...
    if(ied && ied->number_of_functions > 0) {
      ...
      self->exports = (struct export_table_item*)malloc(
          sizeof(struct export_table_item) * self->exports_len);
      byte_zero(self->exports,
                sizeof(struct export_table_item) * self->exports_len);
      addrs =
          pe_rva2ptr(img->base, uint32_get(&ied->address_of_functions));
      ords = pe_rva2ptr(img->base,
                        uint32_get(&ied->address_of_name_ordinals));
      ...
      for(i = 0; i < ied->number_of_functions; i++) {
        if(addrs[i] != 0) {
          ...
          if((idata->virtual_address <= addrs[i]) &&
             (idata->virtual_address + idata->size > addrs[i])) {
            self->exports[i].address = NULL;
            self->exports[i].forward_str =
                str_dup(pe_rva2ptr(img->base, uint32_get(&addrs[i])));
          } else
            self->exports[i].address =
                pe_rva2ptr(img->base, uint32_get(&addrs[i]));
  ...
  if(idata->size > 0 && idata->virtual_address != 0) {
    ...
    if(iid)
      for(i = 0;
          iid[i].characteristics || iid[i].time_date_stamp ||
          iid[i].forwarder_chain || iid[i].name || iid[i].first_thunk;
          i++) {
        ...
        oith = pe_rva2ptr(img->base,
                          uint32_get(&iid[i].original_first_thunk));

        for(j = 0;
            (impaddress =
                 thunk_data_u1_function(img->base, ith, j, cfg)) != 0;
            j++) {
          ...
          imp->orig_address =
              thunk_data_u1_function(img->base, oith, j, cfg);
          if(cfg->on_self) {
            imp->address = impaddress;
          }
          if(oith && imp->orig_address & (1 << (sizeof(uint32) * 8 - 1))) {
            imp->ordinal =
                imp->orig_address & ~(1 << (sizeof(uint32) * 8 - 1));
          } else if(oith) {
            pe_import_by_name* byname =
                pe_rva2ptr(img->base, uint32_get(&imp->orig_address));
            if(byname != NULL)
  ...
  idata =
      &pe_get_datadir(img->base, NULL)[PE_DIRECTORY_ENTRY_DELAY_IMPORT];
  if(idata->size > 0 && idata->virtual_address != 0) {
    idd = pe_rva2ptr(img->base, uint32_get(&idata->virtual_address));
    if(idd)
      for(i = 0;
          idd[i].attributes.all_attributes || idd[i].dll_name_rva ||
          idd[i].module_handle_rva || idd[i].import_address_table_rva ||
          idd[i].import_name_table_rva ||
          idd[i].bound_import_address_table_rva ||
        ...
        if(idd[i].attributes.all_attributes & 0x00000001) {
          ith = pe_rva2ptr(img->base,
                           uint32_get(&idd[i].import_address_table_rva));
          oith = pe_rva2ptr(img->base,
                            uint32_get(&idd[i].import_name_table_rva));
        } else {
...
char
try_map_and_load(char* name,
                 char* path,
                 pe_loaded_image* loaded_image,
                 int required_machine_type) {
  char success = 0;
  size_t sz;
  stralloc sa;
  pe_dos_header* dhdr;
...
*/
  ...
  if(dhdr) {
    ...
    loaded_image->sections =
        pe_header_sections(loaded_image->base,
                           &loaded_image->number_of_sections);
    success = 1;
  }
...
int
build_dep_tree(build_tree_config* cfg,
               char* name,
               struct dep_tree_element* root,
               struct dep_tree_element* self) {
  pe_loaded_image loaded_image;
  ...
  if(cfg->on_self) {
    ...
    loaded_image.sections =
        pe_header_sections(hmod, &loaded_image.number_of_sections);
    loaded_image.base = (void*)hmod;

    if(cfg->machine_type != -1 &&
       (int)loaded_image.file_header->coff_header.machine !=
           cfg->machine_type)
      return 1;
  } else {
    stralloc sa;
    ...
    strlist_foreach_s(cfg->search_paths, dir) {
      if(path_find(dir, str_basename(name), &sa)) {
        stralloc_nul(&sa);
        success = try_map_and_load(str_basename(sa.s),
                                   dir,
                                   &loaded_image,
                                   cfg->machine_type);
        if(success)
  ...
  another process_dep... for(i = 0; i <
  self->exports_len; i++)
  {
    if(self->exports[i]->forward_str !=
  NULL && self-.exports[i]->forward ==
  NULL)
    {
  ...
  *export_name = NULL, *rdot = NULL;
  ...
  export_name[1] <= '9')
        {
          export_ordinal = strtol
  (&export_name[1], NULL, 10);
          export_name = NULL;
        }
        self->exports[i]->forward =
  find_export_forward (forward_str_copy,
  ...
  */
  for(i = 0; i < self->imports_len; i++) {
    if(self->imports[i].mapped == NULL && self->imports[i].dll != NULL &&
      ...
      for(j = 0; j < dll->exports_len; j++) {
        if((self->imports[i].name != NULL &&
            dll->exports[j].name != NULL &&
  ...
  /* By keeping items in the stack we
   * turn it into a list of all
   * processed modules, this should be
   * more effective at preventing us
   * from processing modules multiple
   * times
   */
  /*pop_stack (stack, stack_len,
   * stack_size, name);*/
  return 0;
}
...
print_image_links(int first,
  ...
  if(!unresolved && !first) {
    if(str_case_diff(self->module, self->resolved_module) == 0) {
      ...
      buffer_putm_internal(
          buffer_1, " => ", self->resolved_module, " (0x", NULL);
      buffer_putptr(buffer_1, self->mapped_address);
    }
    ...
    for(i = 0; i < self->imports_len; i++) {
      ...
      buffer_puts(buffer_1,
                  ...
                  : item->dll->module ? item->dll->module
                                      : "<0>");
      buffer_putnlflush(buffer_1);
      /*printf("\t%*s%llx %llx %3d %s %s
         %s\n", depth, depth > 0 ? " " :
         "", (long
         long)item->orig_address, (long
         long)item->address,
             item->ordinal,
             item->name ? item->name :
         "<0>", item->mapped ? "" :
...
#if WINDOWS
/* registry_query("HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session
...
registry_query(const char* key, const char* value, stralloc* sa) {
  HKEY hkey = 0;
  DWORD len, ret, type;
  typedef LONG(WINAPI reggetvalue_fn)(
      HKEY, LPCSTR, LPCSTR, DWORD, DWORD*, void*, DWORD*);
  ...
  if(!str_diffn(key, "HKCU", 4) ||
     !str_diffn(key, "HKEY_CURRENT_USER", 17)) {
    hkey = HKEY_CURRENT_USER;
  } else if(!str_diffn(key, "HKLM", 4) ||
            !str_diffn(key, "HKEY_LOCAL_MACHINE", 19)) {
    hkey = HKEY_LOCAL_MACHINE;
  } else if(!str_diffn(key, "HKCR", 4) ||
            !str_diffn(key, "HKEY_CLASSES_ROOT", 17)) {
    hkey = HKEY_LOCAL_MACHINE;
  } else if(!str_diffn(key, "HKU", 3) ||
            !str_diffn(key, "HKEY_USERS", 10)) {
    hkey = HKEY_LOCAL_MACHINE;
  }
  ...
  len = sa->a;
  ret = (*api_fn)(
      hkey, strchr(key, '\\') + 1, value, RRF_RT_ANY, &type, sa->s, &len);
  sa->len = len;
...
main(int argc, char** argv) {
  int i = 0;
  static int verbose = 0, unused = 0, datarelocs = 0, functionrelocs = 0,
             recursive = 0, list_exports = 0, list_imports = 0;
  int skip = 0;
  int files = 0;
  int files_start = -1;
  int files_count = 0;
...
#ifdef DEBUG_OUTPUT
        buffer_putm_internal(
            buffer_2, "Registry path [", keys[kidx], "]: ", NULL);
  ...
  if(!skip && files_start > 0) {
...

    {
      ...
      for(i = files_start; i < argc; i++) {
        struct dep_tree_element* child = (struct dep_tree_element*)malloc(
            sizeof(struct dep_tree_element));
        byte_zero(child, sizeof(struct dep_tree_element));
