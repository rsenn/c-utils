...
static int print_export_dir, print_data_dir, print_opt_header,
    print_offset_rva, print_rva_offset, print_range, print_as_rva;
...
pe_print_export_directory(buffer* b,
  ...
  buffer_putxlong0(
      b,
      pe_rva2offset(base,
                    uint32_get(&export_dir->address_of_name_ordinals)),
...
pe_dump_exports(uint8* base) {
  ...
  nameptr =
      (uint32*)pe_rva2ptr(base, uint32_get(&exports->address_of_names));
  fnptr = (uint32*)pe_rva2ptr(base,
                              uint32_get(&exports->address_of_functions));
  ordptr =
      (uint16*)pe_rva2ptr(base,
                          uint32_get(&exports->address_of_name_ordinals));
