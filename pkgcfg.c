...
host_arch(const char* compiler, stralloc* out) {
   ...
   */
  const char* pkg_get(pkg * pf, const char* key) {
    char* v = NULL;
    size_t len;
   ...
   */
  int wordexp_sa(const char* s, stralloc* sa, MAP_T vars) {
    const char *x, *v;
    size_t i, n;
    stralloc var;
    stralloc_init(&var);
   ...
   * @return          1 on sucess, 0 on failure
   */
  int pkg_expand(pkg * pf, const char* key, stralloc* out) {
    stralloc k;
    const char* s;
  ...
  static int visit_set(const void* key, size_t key_len, const void* value, size_t value_len, void* user_data) {
    pkg* p = user_data;
    stralloc var, v;
    stralloc_init(&var);
  ...
  static int visit_unset(const void* key, size_t key_len, const void* value, size_t value_len, void* user_data) {
    (void)key_len;
    (void)value;
    (void)value_len;
    (void)user_data;
  ...
  static int visit_dump(const void* key, size_t key_len, const void* value, size_t value_len, void* user_data) {
  ...
  void pkg_init(pkg * pf, const char* fn) {
   ...
   * @return         1 on success, 0 on failure
   */
  int pkg_open(const char* pkgname, pkg* pf) {
    buffer pc;
    const char* s;
    size_t n;
    int ret = 0;
   ...
   */
  int pkg_conf(strarray * modules, int mode) {
    int i;
    stralloc value;
    stralloc_init(&value);
      ...
      switch(c) {
        ...
        case '?': {
          ...
          if(!str_diffn(arg, "--libs", 6) || !str_diffn(arg, "--cflags", 8)) {
            ...
            if(i == PRINT_LIBS)
              libs_mode =
                  arg[str_find(arg, "only")] ? (arg[str_find(arg, "other")] ? LIBS_ONLY_OTHER : LIBS_ONLY_L) : 0;
            else
              cflags_mode =
