...
static char *t1 = 0, *t2 = 0, *t3 = 0;
static char *cname = 0, *referral = 0;
...
int
dd(const char* q, const char* base, char ip[16], bool ip6) {
  int j;
  unsigned int x;
  int iplen = ip6 ? 16 : 4;
...
static void
cachegeneric(const char type[2],
             const char* d,
             const char* data,
...
static int
globalip(char* d, char ip[]) {
  if(dns_domain_equal(d, "\011localhost\0")) {
    byte_copy(ip, 4, "\177\0\0\1");
    return 1;
  }
  if(dd(d, "\0", ip, false) == 4)
    return 1;
  return 0;
}
...
query_aliases(struct query* z) {
  ...
  for(i = QUERY_MAXALIAS - 1; i >= 0; --i)
    if(z->alias[i]) {
      ...
      while(i > 0) {
        if(!response_cname(
               &z->response, z->alias[i], z->alias[i - 1], z->aliasttl[i]))
          return 0;
        --i;
      }
      if(!response_cname(
             &z->response, z->alias[0], z->name[0], z->aliasttl[0]))
        return 0;
      return 1;
    }
...
new_name:
  ...
  if(dlen <= 255) {
    ...
    if(!typematch(DNS_T_ANY, dtype) && !typematch(DNS_T_AXFR, dtype) &&
       ...
       !typematch(DNS_T_PTR, dtype) && !typematch(DNS_T_A, dtype) &&
...
have_ns:
  for(j = 0; j < QUERY_MAXNS; ++j) {
    if(z->ns[z->level][j]) {
      if(z->level + 1 < QUERY_MAXLEVEL) {
        int dc =
            dns_domain_copy(&z->name[z->level + 1], z->ns[z->level][j]);
        if(!dc)
          goto die;
  ...
  if(z->level) {
    ...
    log_tx(z->name[z->level],
           DNS_T_A,
           z->control[z->level],
           z->servers[z->level],
           z->level);

    if(dns_transmit_start(&z->dt,
                          z->servers[z->level],
                          flagforwardonly,
                          z->name[z->level],
                          DNS_T_A,
              ...
              if(datalen == 4)
                for(k = 0; k < 64; k += 4) {
                  if(byte_equal(z->servers[z->level - 1] + k,
                                ...
                                "\0\0\0\0")) {
                    if(!dns_packet_copy(
                           buf, len, pos, z->servers[z->level - 1] + k, 4))
                      goto die;
                    break;
                  }
                }
...
int
query_start(
    struct query* z, char* dn, char type[], char class[], char localip[]) {
...
query_dump(struct query const* q) {
  ...
  for(i = 0; i <= q->level; i++) {
    ...
    buffer_put(buffer_2,
               buf,
               q->name[i] ? dns_domain_todot(buf, q->name[i])
                          : str_copy(buf, "NULL"));
    buffer_puts(buffer_2, "\n\t\tcontrol = ");
    buffer_put(buffer_2,
               buf,
               q->control[i] ? dns_domain_todot(buf, q->control[i])
                             : str_copy(buf, "NULL"));
    buffer_puts(buffer_2, "\n\t\tns =");
    for(j = 0; j < QUERY_MAXNS; j++) {
      if(q->ns[i][j]) {
    ...
    for(j = 0; j < QUERY_MAXNS; j++) {
      if(byte_diff(&q->servers6[i][j * 16], 16, V6any)) {
        ...
        buffer_put(buffer_2,
                   buf,
                   fmt_hexb(buf, &q->servers6[i][j * 16], 16));
      }
    }
