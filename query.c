...
new_name:
  ...
  if(dlen <= 255) {
    ...
    if(!typematch(DNS_T_ANY, dtype) && !typematch(DNS_T_AXFR, dtype) && !typematch(DNS_T_CNAME, dtype) && !typematch(DNS_T_NS, dtype) &&
       !typematch(DNS_T_PTR, dtype) && !typematch(DNS_T_A, dtype) && !typematch(DNS_T_MX, dtype) && !typematch(DNS_T_SOA, dtype)) {
...
int
query_start(struct query* z, char* dn, char type[], char class[], char localip[]) {
