...
int collapse_unicode(char *buffer, unsigned int n) {
 ...
 */  
    ...
    if(key[0])
    {
      unsigned int keystart, keyend, valuestart, valueend;
      regtype_t rt = 0;
      uint64 word = 0;
      int inquote;
...
//			    line.s[keyend] = '\0';
  		...
  		buffer_puts(&buffer_1, "/t ");
 		  buffer_puts(&buffer_1, regtype_strings[rt]);
				...
				case REG_BINARY:  {
					...
					for(pos = valuestart; pos < valueend; pos++) {
						if(scan_fromhex(line.s[pos]) != -1)
							buffer_putc(&buffer_1, tolower(line.s[pos]));
					}
					buffer_putc(&buffer_1, '"');
					break;
				}
