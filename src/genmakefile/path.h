...
void path_prefix_b(const stralloc*, const char* x, size_t n, stralloc* out, char pathsep_make);
void path_prefix_s(const stralloc*, const char* path, stralloc* out, char pathsep_make);
void path_prefix_sa(const stralloc*, stralloc* sa, char pathsep_make);
char* path_extension(const char*, stralloc* out, const char* ext);
char* path_output(const char*, stralloc* out, const char* ext, char pathsep_args);
char* path_wildcard(stralloc*, const char* wildchar);
char* path_clean_s(const char*);
char* path_clean_b(const char*, size_t* len);
const char* path_mmap_read(const char*, size_t* n, char pathsep_make);
