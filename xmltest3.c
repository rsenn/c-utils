 ...
 * test: reader4 test1.xml test2.xml test3.xml > reader4.tmp && diff reader4.tmp
 * $(srcdir)/reader4.res author: Graham Bennett copy: see Copyright for the
...
static void
processDoc(xmlTextReaderPtr readerPtr) {
  int ret;
  xmlDocPtr docPtr;
  const xmlChar* URL;
  ...
  URL = docPtr->URL;
  if(NULL == URL) { buffer_putsflush(buffer_2, "Failed to obtain URL\n"); }
